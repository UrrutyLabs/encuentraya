...
export const BookingCard = memo(function BookingCard({ booking, hasReview = false }: BookingCardProps) {
  const router = useRouter();
  
  // Memoize computed values
  const statusLabel = useMemo(() => getBookingStatusLabel(booking.status), [booking.status]);
  const statusVariant = useMemo(() => getBookingStatusVariant(booking.status), [booking.status]);
  const categoryLabel = useMemo(() => CATEGORY_LABELS[booking.category] || booking.category, [booking.category]);
  const formattedDate = useMemo(() => formatDate(booking.scheduledAt), [booking.scheduledAt]);
  ...
  const handleRebook = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
